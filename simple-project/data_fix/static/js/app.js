                ...
                assignments.forEach(assignment => {
                    ...
                    if (!fieldMatch) {
                        ...
                        if (fieldMatch2) {
                            ...
                            // 更新对应字段
                            if (field === 'START_FLAG') {
                                const oldValue = targetRow.START_FLAG;
                                targetRow.START_FLAG = parseInt(value) || 0;
                                this.updatedFields[rowKey].push({
                                    field: 'START_FLAG',
                                    oldValue: oldValue,
                                    newValue: targetRow.START_FLAG
                                });
                            } else if (field === 'LATEST_FLAG') {
                                const oldValue = targetRow.LATEST_FLAG;
                                targetRow.LATEST_FLAG = parseInt(value) || 0;
                                this.updatedFields[rowKey].push({
                                    field: 'LATEST_FLAG',
                                    oldValue: oldValue,
                                    newValue: targetRow.LATEST_FLAG
                                });
                            }
                        }
                        return;
                    ...
                    // 更新对应字段
                    if (field === 'END_DATE') {
                        const oldValue = targetRow.END_DATE;
                        ...
                        const oldValue = targetRow.START_DATE;
        ...
        rows.forEach(row => {
            ...
            columns.forEach(col => {
                ...
                if (updatedFields) {
                    ...
                    if (updatedFields[rowKey]) {
                        const fieldUpdate = updatedFields[rowKey].find(f => f.field === col);
                        if (fieldUpdate) {
                            cellClass = 'updated-cell';
                            // 显示变更前后的信息
                            cellContent = `${fieldUpdate.oldValue} → ${fieldUpdate.newValue}`;
                        }
                    }
                }
                ...
                if (showChanges && !cellClass) {
                    ...
                    if (originalRow && originalRow[col] !== value) {
                        ...
                        cellContent = `${originalRow[col]} → ${value}`;
                    }
                }
